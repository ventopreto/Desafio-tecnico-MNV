...
RSpec.describe Rental, type: :model do
  let(:pessoa) { Author.create!(name: "Fernando Pessoa", biography: "Um dos maiores poetas da língua portuguesa.") }
  let(:user) { User.create!(name: "Usuario Generico", email: "usuario@example.com") }
  let(:desassossego) do
    Book.create!(title: "Desassossego", synopsis: "Um livro de Fernando Pessoa", published_at: Date.new(1933), author: pessoa)
  end
  ...
  describe "validações" do
    ...
    context "quando end_date está presente" do
      let(:rental) { Rental.new(start_date: Date.today, end_date: Date.yesterday, book: desassossego, user: user) }
